...
const categoryLabels: { [key: string]: string } = {
  mariage: 'Mariage',
  corporate: 'Réunion Pro',
  anniversaire: 'Réception',
  evenement_professionnel: 'Réunion Pro',
};
...
export default function GalleryPage({ onNavigate }: GalleryPageProps) {
  ...
  const getUniqueCategories = () => {
    const categories = portfolios.map(p => p.category);
    const uniqueCategories = Array.from(new Set(categories));
    
    const mappedCategories = uniqueCategories.map(cat => {
      if (cat === 'mariage') return 'Mariage';
      if (cat === 'corporate' || cat === 'evenement_professionnel') return 'Réunion Pro';
      if (cat === 'anniversaire') return 'Réception';
      return 'Autres';
    });
    
    return Array.from(new Set(mappedCategories));
  };

  const filters = ['all', ...getUniqueCategories()];

  const filteredPortfolios = selectedFilter === 'all'
    ? portfolios
    : portfolios.filter(p => {
        const displayCategory = categoryLabels[p.category] || 'Autres';
        return displayCategory === selectedFilter;
      });
  ...
  const toggleGallery = (portfolioId: number) => {
  ...
  return (
    <div className="min-h-screen font-playfair text-[#111827] overflow-x-hidden">
      <style jsx global>{`
        @import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,500;0,600;0,700;1,400&family=Raleway:ital,wght@0,300;0,400;0,500;0,600;0,700;1,300;1,400&display=swap');
      ...
      <div className="pt-32 pb-20 relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-16">
            <div style={{ animation: 'slideDown 0.8s ease-out' }} className="inline-block mb-6">
            ...
            <div className="flex justify-center gap-2" style={{ animation: 'fadeInUp 1s ease-out 0.3s both' }}>
          ...
          <div className="flex flex-wrap justify-center gap-3 mb-16" style={{ animation: 'scaleUp 0.6s ease-out 0.4s both' }}>
            {filters.map((filter, index) => (
              <button
                ...
                style={{ animation: `slideUp 0.6s ease-out ${0.45 + index * 0.08}s both` }}
              >
          ...
          {loading ? (
            <div className="text-center py-20">
              <div className="w-16 h-16 border-4 border-[#ad5945] border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
              <p className="font-inter text-xl text-gray-500">Chargement des portfolios...</p>
            </div>
          ) : (
            <>
              ...
              {filteredPortfolios.length === 0 && (
                <div className="text-center py-20" style={{ animation: 'scaleUp 0.6s ease-out' }}>
